...
function applyHunks(original: string, hunks: Hunk[], accepted: Set<number>): string {
  const a = original === '' ? [] : original.split('\n')
  let ai = 0
  const out: string[] = []
...
function HunkCard({ hunk, accepted, onToggle }: { hunk: Hunk; accepted: boolean; onToggle: () => void }) {
