...
// Packaged builds run from inside Meshflow.app's Resources/app.asar, where
// __dirname-relative math no longer lands on a real checkout. Meshflow is a
...
// to bundle .venv/src — overridable via MESHFLOW_REPO_ROOT if the checkout moves.
 ...
 * MESHFLOW_REPO_ROOT or PACKAGED_REPO_ROOT, since __dirname then points inside
 * the app bundle, not a real checkout.
 */
export function repoRoot(): string {
  if (app.isPackaged) {
    return process.env.MESHFLOW_REPO_ROOT ?? PACKAGED_REPO_ROOT
  }
