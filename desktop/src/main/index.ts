...
// requires commandLine, not a CLI flag). Gated on LAKOORA_CDP_PORT alone (not
...
if (process.env.LAKOORA_CDP_PORT) {
  app.commandLine.appendSwitch('remote-debugging-port', process.env.LAKOORA_CDP_PORT)
}

// Redirect userData to an isolated temp dir during E2E runs so test runs never
// touch the developer's real settings.
if (process.env.LAKOORA_E2E_USER_DATA_DIR) {
  app.setPath('userData', process.env.LAKOORA_E2E_USER_DATA_DIR)
}
...
app.whenReady().then(() => {
  app.setAppUserModelId('com.lakoora.desktop')

  registerAllIPC()

  const mainWindow = createWindow()
  mainWindow.setTitle('Lakoora')
