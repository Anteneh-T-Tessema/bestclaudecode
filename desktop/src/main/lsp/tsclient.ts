...
// Minimal LSP JSON-RPC client over stdio for typescript-language-server.
// Mirrors pyrightClient.ts exactly, adjusted for TS language IDs and init options.
...
function langIdForUri(uri: string): string {
  const ext = uri.split('.').pop()?.toLowerCase() ?? ''
  return LANG_IDS[ext] ?? 'typescript'
}

class TSClient extends EventEmitter {
  ...
  start(): Promise<void> {
    ...
    this.startPromise = new Promise((resolve, reject) => {
      const binPath = path.join(repoRoot(), 'desktop', 'node_modules', '.bin', 'typescript-language-server')
      ...
      this.request('initialize', {
        ...
        initializationOptions: {
          tsserver: { logDirectory: null, logVerbosity: 'off' },
        },
      })
        .then(() => {
  ...
  async didOpen(uri: string, text: string): Promise<void> {
    ...
    this.notify('textDocument/didOpen', {
      textDocument: { uri, languageId: langIdForUri(uri), version: 1, text },
    })
  }
...
export const tsClient = new TSClient()
